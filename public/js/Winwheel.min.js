            ...
            this.ctx.font = m; this.ctx.fillStyle = n; this.ctx.strokeStyle = r; this.ctx.lineWidth = f; a = g.text.split("\n"); b = -(a.length / 2 * c) + c / 2; "curved" != d || "inner" != e && "outer" != e || (b = 0); for (f = 0; f < a.length; f++) {
                if ("reversed" == t) if ("horizontal" == d) this.ctx.textAlign = "inner" == e ? "right" : "outer" == e ? "left" : "center", this.ctx.textBaseline = "middle", m = this.degToRad(g.endAngle - (g.endAngle - g.startAngle) / 2 + this.rotationAngle - 90 - 180), this.ctx.save(), this.ctx.translate(l, p), this.ctx.rotate(m), this.ctx.translate(-l, -p), "inner" == e ?
                    ...
                    -p), "inner" == e ? (n && this.ctx.fillText(a[f], l + v + h, p + b), r && this.ctx.strokeText(a[f], l + v + h, p + b)) : "outer" == e ? (n && this.ctx.fillText(a[f], l + w - h, p + b), r && this.ctx.strokeText(a[f], l + w - h, p + b)) : (n && this.ctx.fillText(a[f], l + v + (w - v) / 2 + h, p + b), r && this.ctx.strokeText(a[f], l + v + (w - v) / 2 + h, p + b)), this.ctx.restore(); else if ("vertical" == d) {
                        this.ctx.textAlign = "center"; this.ctx.textBaseline = "inner" == e ? "bottom" : "outer" == e ? "top" : "middle"; m = g.endAngle - (g.endAngle - g.startAngle) / 2; m += this.rotationAngle; this.ctx.save(); this.ctx.translate(l, p); this.ctx.rotate(this.degToRad(m)); this.ctx.translate(-l, -p); k = 0; "outer" == e ? k = p - w + h : "inner" == e && (k = p - v - h); m = c - c / 9; if ("outer" == e) for (q = 0; q < a[f].length; q++)u = a[f].charAt(q), n && this.ctx.fillText(u, l + b, k), r && this.ctx.strokeText(u, l + b, k), k += m; else if ("inner" == e) for (q = a[f].length - 1; 0 <= q; q--)u = a[f].charAt(q), n && this.ctx.fillText(u, l + b, k), r && this.ctx.strokeText(u, l + b, k), k -= m; else if ("center" == e) for (k = 0, 1 < a[f].length && (k = m * (a[f].length - 1) / 2), k = p - v - (w - v) / 2 - k - h, q = 0; q < a[f].length; q++)u = a[f].charAt(q), n && this.ctx.fillText(u,
...
function winwheelAnimationLoop() {
  if (winwheelToDrawDuringAnimation) {
    // KHÔNG vẽ lại nếu modal congratulation đang mở
    if (document.querySelector('#resultModal.show')) return;
    if (winwheelToDrawDuringAnimation.animation.clearTheCanvas) {
    ...
    var before = winwheelToDrawDuringAnimation.animation.callbackBefore;
    var after = winwheelToDrawDuringAnimation.animation.callbackAfter;
    if (before) ("function" === typeof before ? before() : eval(before));
    winwheelToDrawDuringAnimation.draw(false);
    if (after) ("function" === typeof after ? after() : eval(after));
...
function winwheelResize() {
  if (!winwheelToDrawDuringAnimation) return;

  let resizeTimeout;
  clearTimeout(resizeTimeout);
  resizeTimeout = setTimeout(() => {
    let margin = 40;
    if ("undefined" !== typeof winwheelToDrawDuringAnimation._responsiveMargin) {
    ...
    targetWidth = Math.max(minWidth, Math.min(targetWidth, winwheelToDrawDuringAnimation._originalCanvasWidth));
    let scale = targetWidth / winwheelToDrawDuringAnimation._originalCanvasWidth;
    winwheelToDrawDuringAnimation.canvas.width = winwheelToDrawDuringAnimation._originalCanvasWidth * scale;

    if (winwheelToDrawDuringAnimation._responsiveScaleHeight) {
      let targetHeight = winwheelToDrawDuringAnimation._originalCanvasHeight * scale;
      targetHeight = Math.max(minHeight, Math.min(targetHeight, winwheelToDrawDuringAnimation._originalCanvasHeight));
      winwheelToDrawDuringAnimation.canvas.height = targetHeight;
    }

    winwheelToDrawDuringAnimation.scaleFactor = scale;
